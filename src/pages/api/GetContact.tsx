...
const codeExamples: Record<Language, string> = {
  "cURL": `# Get contact by ID
curl -X GET 'https://api.monosend.io/contacts/294e3bfc-e8f8-4b51-a1f1-d71872b8db4e' \\
  -H 'Authorization: Bearer mono_xxxxx'

# Get contact by email
curl -X GET 'https://api.monosend.io/contacts/mark.tsukerberg@gmail.com' \\
  -H 'Authorization: Bearer mono_xxxxx'`,
...
const GetContact = () => {
  ...
  return (
    <main className="flex max-w-[1600px] mx-auto px-6">
      <ApiSidebar activePath="/api/get-contact" />

      <div className="flex-1 min-w-0 px-12 py-8">
        <div className="text-sm text-muted-foreground mb-4">Contacts</div>

        <div className="flex items-start justify-between mb-4">
          <h1 className="text-3xl font-bold text-foreground">Get Contact</h1>
          <CopyPageDropdown />
        </div>

        <p className="text-muted-foreground mb-6">
          Retrieve a contact by its ID or email address.
        </p>

        <div className="mb-8">
          <Endpoint method="GET" url="https://api.monosend.io/contacts/{contact_id}" />
        </div>
