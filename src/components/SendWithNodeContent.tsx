...
const SendWithNodeContent = () => {
  return (
    <main className="flex-1 min-w-0 py-8 px-12">
      <div className="mb-4">
        <span className="text-xs font-semibold text-primary uppercase tracking-wider">Node</span>
      </div>

      <div className="flex items-start justify-between mb-6">
        <h1 className="text-3xl sm:text-4xl font-semibold text-foreground">Send emails with Node</h1>
        <CopyPageDropdown />
      </div>

      <p className="text-base text-muted-foreground mb-8 leading-relaxed">
        Learn how to send your first email using the MonoSend Node SDK.
      </p>
      ...
      <section id="1-install" className="mt-10 scroll-mt-32">
        ...
        <p className="text-base text-muted-foreground mb-4 leading-relaxed">
          Add the MonoSend SDK to your Node app.
        </p>
      ...
      <section id="2-send-email-using-html" className="mt-10 scroll-mt-32">
        ...
        <p className="text-base text-muted-foreground mb-4 leading-relaxed">
          Use the HTML parameter to send your first email.
        </p>
        <div className="rounded-xl border border-border bg-muted/40 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2 border-b border-border text-xs font-semibold text-muted-foreground">
            <span>index.js</span>
...
const client = new MonoSend(process.env.MONOSEND_API_KEY);
...
await client.emails.send({
  ...
  html: "<strong>It works!</strong>",
      ...
      <section id="3-try-it-yourself" className="mt-10 scroll-mt-32">
        ...
        <a
          href="https://github.com/monosend"
          className="block rounded-xl border border-border p-4 hover:bg-muted transition-colors"
        >
          <h3 className="text-base font-semibold text-foreground mb-1">Node Example</h3>
          <p className="text-sm text-muted-foreground">Explore more Node SDK examples.</p>
        </a>
      </section>
...
export default SendWithNodeContent;
